...
describe('pilot installation entry points', () => {
  ...
  test('the agent obtains exact package identity without a user receipt or authentication', () => {
    ...
    const download = install.slice(install.indexOf('### Pilot download'), install.indexOf('Success looks like'))
      .replace(/\s+/g, ' ');
    expect(download).toContain('https://api.github.com/repos/jamiezigelbaum/Olympus/releases/tags/v0.4.0-beta.4');
    expect(download).toContain('https://github.com/jamiezigelbaum/Olympus/releases/download/v0.4.0-beta.4/olympus-0.4.0-beta.4.tgz');
    ...
    expect(download).toContain('e6b8d16b8062f625cb8f1ffc31e695ca1771bc960ac728252135941e53aa3976');
    expect(download).toContain('Byte count: `1181923`');
