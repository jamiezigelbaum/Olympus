...
describe('Google pilot client resolution order', () => {
  ...
  test('an unconfigured release identity falls back to the shipped default', () => {
  ...
  test('source does not invent an unconfigured client identity', () => {
    expect(PACKAGED_GOOGLE_PILOT_CLIENT_ID).toBe('');
  });
