...
describe('operations', () => {
  test('defines the current operation surface', () => {
    // Every registered operation is on the v0.4 public tool lists. The fifteen
    // private tools (pre-v0.4 email, index administration, unqualified source
    // workflows) were deleted on 2026-09-18; test/public-surface-guard.test.ts
    // is what keeps the registry and the public lists equal from here.
    ...
    expect(operations.find((operation) => operation.name === 'source_watch_create')?.mutating).toBe(true);
    expect(operations.find((operation) => operation.name === 'source_watch_cancel')?.mutating).toBe(true);
    expect(operations.find((operation) => operation.name === 'source_watches')?.mutating).toBe(false);
  });
  ...
  test('source_answer delegates Castor-safe source parameters to private source lane', async () => {
    ...
    expect(operationToolSchema(sourceAnswer!)).toMatchObject({
      properties: {
        corpus_id: { enum: ['secure_local.email.private', 'internal.email', 'public_safe.email', 'internal.drive.docs', 'secure_local.drive.docs', 'public_safe.drive.docs', 'internal.telegram.messages', 'internal.readwise.library', 'secure_local.readwise.library', 'internal.x.bookmarks', 'secure_local.x.bookmarks', 'secure_local.dropbox.files', 'internal.dropbox.files', 'public_safe.dropbox.files', 'secure_local.telegram.protected.messages', 'secure_local.whatsapp.messages', 'internal.whatsapp.messages'] },
