...
function defaultSovereigntyConfigPath() {
  return join7(homedir5(), ".olympus", "sovereignty.json");
}
...
function defaultHandleRegistryPath() {
  return join8(homedir6(), ".config", "olympus", "handles.json");
}
...
function defaultSensitivityMapPath() {
  return join10(homedir8(), ".olympus", "sensitivity-map.json");
}
...
function defaultSourceDashboardHistoryDbPath(env = process.env) {
  ...
  return join12(dataHome, "openclaw", "olympus", "source-dashboard.sqlite");
}
...
function defaultDetachedOAuthStateDir() {
  return join9(homedir7(), ".olympus", "pending-oauth");
}
...
function doctorSovereigntyConfigPath(deps) {
  ...
  return home ? join13(home, ".olympus", "sovereignty.json") : undefined;
}
...
function ingestionHealthStatePath(deps) {
  ...
  return join13(dirname10(defaultSourceDashboardHistoryDbPath(deps.env)), "source-ingestion-doctor-state.json");
}
...
function defaultCommandExists(command) {
  ...
  return path.split(":").some((dir) => Boolean(dir) && existsSync9(join13(dir, command)));
}
...
var plugin = {
  ...
  register(api) {
    ...
    } else if (config.worker.service.enabled || config.worker.telegramCapture.enabled || config.worker.whatsappCapture.enabled) {
