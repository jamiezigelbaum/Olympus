...
import { chmodSync, existsSync as existsSync4, mkdirSync as mkdirSync4, readFileSync as readFileSync8, writeFileSync as writeFileSync3 } from "node:fs";
...
function parseSovereigntyConfig(value, label) {
  ...
  const retrievalRecord = asRecord10(record.retrieval);
  const trustDomainsRecord = asRecord10(retrievalRecord?.trustDomains);
  const trustDomains = {};
  for (const domain of BUILTIN_DOMAINS) {
    const policy = asRecord10(trustDomainsRecord?.[domain]);
    if (policy)
...
function unwrapSovereignty(value) {
  ...
  if (record?.sovereignty && asRecord10(record.sovereignty)?.schemaVersion === SOVEREIGNTY_SCHEMA_VERSION) {
...
function parseRoutes(value, label) {
  ...
  const routes = {};
  for (const domain of BUILTIN_DOMAINS) {
    const route = asRecord10(record[domain]);
    if (!route)
      continue;
    const legacyAnalyst = route.analyst;
    const poolRecord = asRecord10(route.pool);
...
import { existsSync as existsSync6, mkdirSync as mkdirSync5, readFileSync as readFileSync9 } from "node:fs";
...
import { chmodSync as chmodSync2, existsSync as existsSync8, lstatSync as lstatSync2, readFileSync as readFileSync11 } from "node:fs";
...
function loadSensitivityMap(options = {}) {
  ...
  try {
    return parseSensitivityMap(JSON.parse(readFileSync11(path, "utf8")), path);
  } catch (error) {
...
function sensitivityMapRemedy(path) {
  return `Write the map to ${path}. Run olympus setup first if ${dirname10(path)} does not exist yet; it creates that directory with owner-only permissions.`;
}
function parseSensitivityMap(rawMap, label = "sensitivity map") {
  const root = asRecord11(rawMap);
  if (!root)
    throw new OperationError("config_error", `${label} must be an object.`);
...
function assertUserFacingTierMapping(value, label) {
  ...
  for (const tierName of USER_FACING_TIER_NAMES) {
    const mapped = asRecord11(record[tierName]);
...
function parseCategory(value, label) {
  ...
  const matchRecord = asRecord11(record.match);
  if (!matchRecord)
...
class RestGmailApiClient {
  ...
  async listMessages(request) {
    ...
    const record = asRecord12(json, "Gmail messages list response");
    return {
      messages: Array.isArray(record.messages) ? record.messages.map((item) => asRecord12(item, "Gmail message list item")).map((item) => ({
...
class RestGoogleDriveApiClient {
  ...
  async listFiles(request) {
    ...
    const record = asRecord13(json, "Google Drive files list response");
    return {
      files: Array.isArray(record.files) ? record.files.map((item) => normalizeDriveFile(asRecord13(item, "Google Drive file"))).filter((file) => file.id) : [],
  ...
  async getFolder(folderId) {
    ...
    const record = asRecord13(json, "Google Drive folder");
...
function normalizeDriveFile(record) {
  return {
    ...
    ...Array.isArray(record.owners) ? { owners: record.owners.map((owner) => asRecord13(owner, "Google Drive owner")).map((owner) => optionalStringProp2(owner, "emailAddress")) } : {}
  };
}
...
import { accessSync, constants, readFileSync as readFileSync6, statSync as statSync5 } from "node:fs";
import { homedir as homedir4 } from "node:os";
import { isAbsolute as isAbsolute4, join as join6 } from "node:path";
var SERVICE_ID3 = "olympus-whatsapp-capture";
...
import { delimiter as delimiter2, dirname as dirname5, isAbsolute as isAbsolute5, join as join8 } from "node:path";
...
// src/core/native-embedding-drain-service.ts
var SERVICE_ID4 = "olympus-source-embedding-drain";
...
function resolveBunRuntimePath2(configured, env) {
  ...
  for (const candidate of candidates) {
    if (!candidate || !isAbsolute5(candidate))
      continue;
...
async function embeddingDrainReadinessProbe(readinessPath, instanceId, child) {
  try {
    ...
    if (!stat2.isFile() || stat2.size > 16 * 1024)
      return false;
    const receipt = JSON.parse(readFileSync7(readinessPath, "utf8"));
...
import { existsSync as existsSync9, mkdirSync as mkdirSync7, readFileSync as readFileSync12, writeFileSync as writeFileSync5 } from "node:fs";
...
// src/core/connect.ts
import { mkdirSync as mkdirSync6, readFileSync as readFileSync10, rmSync as rmSync2, writeFileSync as writeFileSync4 } from "node:fs";
...
async function workerCredentialReadiness(deps) {
  try {
    ...
    const readiness = asRecord14(body.credential_readiness);
    const policy = asRecord14(readiness.policy);
    ...
    return readiness.ready_profiles.flatMap((entry) => {
      const profile = asRecord14(entry);
...
async function emailWorkerCheck(deps) {
  ...
  const health = asRecord14(await response.json());
...
async function sourceIndexStatusCheck(deps) {
  ...
  const status = asRecord14(await response.json());
  ...
  for (const entry of corpora) {
    ...
    const counts = asRecord14(corpus.counts);
    const embeddingParity = asRecord14(corpus.embedding_parity);
...
async function dropboxContentExtractionThroughputCheck(deps) {
  ...
  if (!signal) {
    const corpus = (Array.isArray(status.corpora) ? status.corpora : []).map((entry) => asRecord14(entry)).find((entry) => entry.corpus_id === DROPBOX_FILES_CORPUS_ID2);
    const counts = asRecord14(corpus?.counts);
...
function degradedCredentialDetails(record, options = {}) {
  ...
  return credentials.flatMap((entry) => {
    const credential = asRecord14(entry);
...
async function sourceSchedulerStatusCheck(deps) {
  ...
  for (const entry of sources) {
    ...
    for (const taskEntry of tasks) {
      const task = asRecord14(taskEntry);
...
async function fetchSourceIndexStatusForIngestion(deps, baseUrl) {
  try {
    ...
    if (!response.ok)
      return;
    return asRecord14(await response.json());
  } catch {
    return;
  }
}
...
function sourceIngestionLedgerFromStatus(status) {
  const ledger = asRecord14(status.ingestion_ledger);
...
function ingestionHealthStatePath(deps) {
  ...
  return join15(dirname12(defaultSourceDashboardHistoryDbPath(deps.env)), "source-ingestion-doctor-state.json");
}
...
function readIngestionHealthState(path) {
  try {
    ...
    const record = asRecord14(parsed);
    const sources = asRecord14(record.sources);
    const normalized = {};
    for (const [sourceId, sourceValue] of Object.entries(sources)) {
      const source = asRecord14(sourceValue);
      const terminal = asRecord14(source.failed_terminal_by_class);
      normalized[sourceId] = {
...
function writeIngestionHealthState(path, state) {
  mkdirSync7(dirname12(path), { recursive: true });
...
async function sourceIndexCorpusIdsForDoctor(deps, baseUrl) {
  try {
    ...
    const status = asRecord14(await response.json());
    const corpora = doctorVisibleCorpora(deps, Array.isArray(status.corpora) ? status.corpora : []);
    return new Set(corpora.map((entry) => asRecord14(entry)).map((corpus) => typeof corpus.corpus_id === "string" ? corpus.corpus_id : undefined).filter((corpusId) => !!corpusId));
  } catch {
    return new Set;
  }
}
...
function staleRunningSync(corpus) {
  const lastRefresh = asRecord14(corpus.last_refresh);
...
function hasSyncRecord(corpus) {
  ...
  if (Object.keys(lastRefresh).length > 0)
    return true;
  const lastSync = asRecord14(corpus.last_sync);
...
function defaultCommandExists(command) {
  ...
  return path.split(":").some((dir) => Boolean(dir) && existsSync9(join15(dir, command)));
}
...
var plugin = {
  ...
  register(api) {
    ...
    } else if (config.worker.service.enabled || config.worker.telegramCapture.enabled || config.worker.whatsappCapture.enabled || config.worker.embeddingDrain.enabled) {
