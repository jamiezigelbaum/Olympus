...
/**
 * The official Venice billing origin. It is a constant on purpose: callers may
 * choose when to probe, never where a resolved provider credential is sent, so
 * a configurable base URL can never forward the key to another host.
 */
export const VENICE_BILLING_BASE_URL = 'https://api.venice.ai/api/v1';
const DEFAULT_BASE_URL = VENICE_BILLING_BASE_URL;
...
export type VeniceCreditFetch = (url: string, init: RequestInit) => Promise<Response>;

export interface FetchVeniceCreditStatusOptions {
  /**
   * Credential source. When supplied, ONLY these names are consulted: the
   * monitor never falls back to the ambient process environment.
   */
  ...
  baseUrl?: string;
  timeoutMs?: number;
  /** Caller-owned cancellation (service stop/restart) forwarded to the request. */
  signal?: AbortSignal;
}
...
/**
 * Publish the status report atomically and privately: the temp file carries
 * mode 0600 from birth, so a reader either sees the previous report or the new
