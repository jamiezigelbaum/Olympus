...
| WhatsApp | paired_session: one linked user device | live linked-device traffic; optional exports; exclude Status broadcasts | Packaged Whatsmeow bridge (Go and a C compiler for its first build) (QR pairing and live capture) | Bridge downtime creates an unrecoverable capture gap; general media-byte extraction is unsupported. | message text; link previews; reactions; media metadata; voice-note transcript sidecars |
...
| Readwise | One-account API-key entry, reconnect, readiness, and bounded Disconnect are available in the dashboard | Resumable Reader v3 and Export v2 traversal with watermarks and daily request guard; category/location capability is explicit and rule authoring remains local | Reader document/highlight text or HTML, notes, author, tags, URL, category, and location; no attachment extraction | The generic scheduler, connector-store pull/reconcile, a separate embedding task with backoff, and canonical hybrid reads in both tiers are the only runtime (owner decision 2026-09-24, embedding ledger `decision-2026-09-24-readwise-hybrid`); the legacy index and migration tooling are deleted | Qualify real-account category/location coverage, provider deletion behavior, citations, reconnect, and clean-install lifecycle in Slice 4 |
...
|---|---|---|---|
| OpenClaw | Native source answer, status, search/locators, and durable watches; CLI lifecycle and dashboard | Repository-only operator and private-deployment tools | All packaged sovereignty presets remain product-supported; v0.4 `private-cloud-only` uses the ordinary Venice API with a live-catalog Private or plain TEE model and supports a separately approved Venice Private embedding model when no local provider is configured. Existing installations require explicit activation with cost approval and a ledger receipt; real-provider embedding qualification is still pending. Olympus does not provide or qualify E2EE out of the box; custom integrations are user-owned. Real-provider qualification is Slice 4. |
