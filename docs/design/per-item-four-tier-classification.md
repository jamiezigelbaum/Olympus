...
| `run --plan <id> [--batch source:…\|folder:…\|label:…\|sender:…\|chat:…] [--max-items <n>]` | M3/M4 | Per batch: `re_embed_started`, then the moves (legacy store bound to the set ledger, placement adopted, move primitive, flip, previous copies superseded), then `re_embed_completed` with the observed chunks handed to each destination's existing model (the drain embeds them). Stops before any destination store's approved chunks or cost would be exceeded (`note`; a resume cannot pass that point, a new approved plan can). |
| `rollback --batch <id>` | M5 | A ledger flip back per item; a `note`. Nothing is re-embedded or deleted, and a Secrets outcome is never made visible again. |
| `purge [--plan <id>] [--approve --expect <digest> --why <reason>]` | M6 | Without `--approve`, counts only, with a digest of those counts. With it (and that digest): an owner-approved `invalidation` entry (pending), then the superseded copies this migration left tombstoned and the text and vectors kept in names-only copies it re-layered stripped, then an `invalidation` entry with the deleted chunks per corpus. |
| `status` | — | Nothing. |
...
- What happens to a Secrets item's copies is decided in ONE place, `SECRETS_DISPOSITION` in `src/workers/connector-store/secrets-disposition.ts`, read by the sync, the extraction landing, the move primitive and the migration alike: `tombstone_now` (today's behavior, §4.2) or `hide_until_purge`. The migration follows it exactly as a sync does, and writes an `invalidation` entry per batch for what it deleted.
- A split raise re-layers the lower store's whole copy to names only; its text and vectors stay there, unserved. Plan (`namesOnlyKeptChunks`) and status (`names_only_kept_chunks`) count them, and the approved purge strips them.
- The doctor excuses embedding lag only while an approved plan is running or stopped, only in its destination stores, and only up to the plan's chunks for that store. A stopped plan is excused for 7 days after it stopped (`OLYMPUS_TIER_MIGRATION_STOPPED_GRACE_DAYS`); after that its lag is reported as usual, noting how many days ago the migration stopped.
- Each destination store is held to its own approved chunks and cost, never to slack approved for another store. A store approved for copied vectors only (0 chunks to embed) receives no embed at all: if its vectors turn out uncopyable at run time, the batch stops (`copy_only_destination_needs_embed`) and nothing more runs until the owner plans again and approves the new costs. A run also refuses when a destination's embedding identity changed since planning, or when a move would hand chunks to a destination the plan never priced.
- A legacy item is adopted only while its ledger row is still what the plan saw; a row a sync recorded or re-decided since then is skipped and left legacy for the next plan, so a crash right after adoption always resumes. The ledger itself refuses to roll back a Secrets row.
